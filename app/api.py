...
@router.post("/orders/{order_id}/dispatch", response_model=OrderDTO)
async def dispatch_order(order_id: int) -> OrderDTO:
    """Async route + sync_to_async.
...
@router.post("/orders/{order_id}/dispatch-sync", response_model=OrderDTO)
def dispatch_order_sync(order_id: int) -> OrderDTO:
    """Plain sync route.

    FastAPI runs sync routes on its own threadpool (40 threads by default),
    so requests run in parallel and each thread gets its own Django connection.
    """
    try:
